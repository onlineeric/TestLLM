...
finetuned_slightly_model = AutoModelForCausalLM.from_pretrained(ft_model_dir, local_files_only=True)
finetuned_slightly_model.to('cuda')
finetuned_slightly_tokenizer = AutoTokenizer.from_pretrained(ft_model_dir, local_files_only=True)
...
while True:
	...
	print(inference(user_input, hf_model, hf_tokenizer, 1000, 512))
	print("$$$ Finetuned slightly model's answer: ")
	print(inference(user_input, finetuned_slightly_model, finetuned_slightly_tokenizer, 1000, 512))